...
class ApplicationStatus(models.Model):
    ...
    new_user_email_template = fields.Many2one("mail.template", string="New user Accepted Admission")
    existed_user_email_template = fields.Many2one("mail.template", string="Existed user Accepted Admission")

    current_year_status_to_facts = fields.Many2one("school_base.enrollment.status", string="Current status to FACTS")
    current_year_next_status_to_facts = fields.Many2one("school_base.enrollment.status", string="Next status to FACTS")

    next_years_status_to_facts = fields.Many2one("school_base.enrollment.status", string="Current status to FACTS")
    next_years_next_status_to_facts = fields.Many2one("school_base.enrollment.status", string="Next status to FACTS")
...
class Application(models.Model):
    ...
    is_current_school_year = fields.Boolean(string="Current school year", compute='_compute_defaults_school_year', search='_search_current_school_year')
    is_enrollment_school_year = fields.Boolean(string="Enrollment school year", compute='_compute_defaults_school_year', search='_search_enrollment_school_year')
    ...
    def _compute_defaults_school_year(self):
        for application_id in self:
            ...
            application_id.is_enrollment_school_year = (application_id.school_year
                                                     or current_school_year
    ...
    # houses
    # home_address_ids = fields.One2many(related="family_id.home_address_ids", string="House addresses")
    ...
    def get_optional_fields(self):
        application_optional_fields_str = self.env['ir.config_parameter'].sudo().get_param('adm_application_optional_field_ids', '')
    ...
    def _compute_application_fields(self):
        for application_id in self:
            ...
            if required_field_ids and optional_field_ids:
                application_id.total_fields_completed = (application_id.fields_completed * 100) / (len(required_field_ids) + len(optional_field_ids))
            else:
    ...
    def create(self, values):
        ...
        application_id.message_subscribe(partner_ids=application_id.partner_id.family_ids.member_ids.filtered(lambda m: m.person_type == 'parent').ids)
            ...
            if values.get('status_id', False):
                ...
                if not self._context.get('forcing', False):
                    if not application_id.state_tasks & application_id.task_ids == application_id.state_tasks and application_id:
                        raise exceptions.ValidationError("All task are not qcompleted")
            else:
