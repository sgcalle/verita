...
class AdmApplicationUserAccess(models.Model):
    ...
    @api.model
    def create(self, vals):
        if vals.get('admin', False):
            vals['page_access_ids'] = [(
                6, 0, self.env['adm.application.page'].search([]).ids)]
        return super(AdmApplicationUserAccess, self).create(vals)

    def write(self, vals):
        if vals.get('admin', False):
            vals['page_access_ids'] = (
                6, 0, self.env['adm.application.page'].search([]).ids)
        return super(AdmApplicationUserAccess, self).write(vals)
