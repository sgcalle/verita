...
class CreateReenrollmentPackage(models.TransientModel):
    ...
    already_imported_student_ids = fields.Many2many('res.partner', compute='_compute_already_imported_studets',
                                                    required=True)
    school_year_id = fields.Many2one('school_base.school_year',
                                     default=lambda self: self.env.company.enrollment_school_year_id)
    ...
    def onchange_school_year_id(self):
        for record in self:
            record.already_imported_student_ids = self.env['adm.reenrollment'].search(
                [('school_year_id', '=', record.school_year_id.id)]).mapped('partner_id')
    ...
    def import_students(self):
        ...
        for student in self.student_ids:
            ...
            try:
                ...
                logger.info("Error with create reenrollment packaget to student[%s] %s: %s" % (student.id, student.name, e))
    ...
    def _build_reenrollment_params(self, student):
        ...
        relationship_custodials = student.self_relationship_ids.filtered(
            lambda r: r.custody and r.partner_relation_id.email)
        # Create users if they doesn't exists
        user_list = []
        for relationship_custodial in relationship_custodials:
            custodial_partner = relationship_custodial.partner_relation_id
